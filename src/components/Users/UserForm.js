...
function UserForm({ handleSubmit, setToggled }) {
  const userValidation = Yup.object().shape({
    email: Yup.string().email("Por favor informe email valido.").required("Por favor informe um email."),
  });
          ...
          {({ isSubmitting, errors, touched, values }) => (
            <Form>
              ...
              <div className="edit-buttons-box col-10 col-md-6 m-4">
                ...
                <div className="btn btn-warning" onClick={() => setToggled(false)}>Fechar</div>
              </div>
              </div>
            </Form>
          )}
